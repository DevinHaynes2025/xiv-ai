...
export type EcosystemAlignmentReport = { protocol:{version:string;evidenceMode:'SIGNED_RECEIPTS_ONLY';authorityPath:string[];universalInstallationClaimed:false;vendorPartnershipsClaimed:false;chipEmbeddingClaimed:false;automaticAccountAccess:false;automaticAgentAuthority:false};targets:{target:string;layer:string;state:'NOT_CONFIGURED'|'REVIEW_REQUIRED'|'CONFIGURED';reason:string;compatibilityTarget:true;partnershipClaimed:false;productionLive:false;installedOnDevices:false;grantsAuthority:false}[];layers:{layer:string;total:number;configured:number;reviewRequired:number;notConfigured:number}[];summary:{targetCount:number;configuredCount:number;productionLiveCount:0;partnershipCount:0;universallyInstalled:false};assurance:{receiptTrust:'SIGNED_RECEIPTS_ONLY';trustStore:'NOT_CONFIGURED'|'LOCAL_REFERENCE'|'QUARANTINED';auditChain:'NOT_CONNECTED'|'VERIFIED_CHECKPOINT'|'FRESH_CHECKPOINT_REQUIRED'|'INVALID';alignmentEvidenceUsable:boolean;independentReview:'REQUIRED';productionActivation:'DISABLED';publicDetail:'AGGREGATES_ONLY'} };
...
export async function requestEcosystemAlignmentReport(accessToken:string):Promise<EcosystemAlignmentReport>{
  ...
  if(!response.ok||!payload||!('summary'in payload)||!('assurance'in payload)||!('layers'in payload)||!Array.isArray(payload.layers)||payload.summary.universallyInstalled!==false||payload.assurance.productionActivation!=='DISABLED'||typeof payload.assurance.alignmentEvidenceUsable!=='boolean'){const code=response.status===401?'unauthorized':'malformed';throw new XivAiRequestError(code,friendlyMessage(code));}
  return payload;
}
