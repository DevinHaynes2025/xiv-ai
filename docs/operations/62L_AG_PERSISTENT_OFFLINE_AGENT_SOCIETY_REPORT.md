...
| US-AG12 Baseline-vs-candidate | **DONE** | Stored baseline vs candidates. More agents / same metrics → `UNAVAILABLE` as an intelligence claim; verdict **UNCHANGED**. Better evidence/calibration → **IMPROVED** with `intelligenceGainClaimed: false`. | Never claims “smarter because more agents”. |
