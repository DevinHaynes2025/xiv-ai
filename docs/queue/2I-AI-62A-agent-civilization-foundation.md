...
**The fork has since materialized.** 2I-AI-62B landed **before** 62A and added a ten-table `xiv_agent_meetings` family. The name clash with the existing `agent_meetings` was dodged by prefixing `xiv_`, but there are now **two parallel meeting schemas** in the same database and **three** homes for message data (`ai_agent_messages`, `agent_mc_messages`, `xiv_agent_meeting_messages`). That makes the reconciliation pass more necessary, not less.
