...
try { Invoke-RestMethod -Uri 'http://127.0.0.1:11434/api/version' -TimeoutSec 2 | Out-Null; $ollamaReachable=$true } catch { $ollamaReachable=$false }
$receipt=[ordered]@{generatedAt=(Get-Date).ToString('o');localhostOnly=$true;runtimeWritable=(Test-Path $root);ollamaReachable=$ollamaReachable;cpuDetected=$true;gpuVerified=$false;productionMutationAllowed=$false;externalNetworkByDefault=$false;status=if($ollamaReachable){'DEGRADED'}else{'UNVERIFIED'};note='HEALTHY requires fresh agent/checkpoint receipts; GPU remains unverified until measured.'}
