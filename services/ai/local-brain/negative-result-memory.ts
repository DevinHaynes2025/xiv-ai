...
import type { EvidenceState, ExperimentKind } from './autonomous-research-types';
...
export async function listNegativeResults(input: {
  tenantId: string;
  universeId: string;
  root?: string;
}): Promise<NegativeResultRecord[]> {
...
export function evidenceStateOf(value: string): EvidenceState {
  if (value === 'PASS' || value === 'FAIL' || value === 'UNAVAILABLE' || value === 'WAITING_DATA' || value === 'UNKNOWN' || value === 'NOT_TESTED') {
    return value;
  }
  return 'UNKNOWN';
}
