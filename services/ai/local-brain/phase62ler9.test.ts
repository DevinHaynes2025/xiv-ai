...
test('bootstrap + soft-wire + cycle; ER1/ER2/ER5/ER6 PRESENT; absent→WAITING_DATA ok', () => {
  ...
  assert.equal(soft.er6HistoricalBusinessCaseAtlasV2.present, true);
  assert.equal(soft.eq16SoftwareWormholeRouter.present, true);
  // EQ14 absent → WAITING_DATA (not FAIL). ER7/ER8 may be mid-flight PRESENT ≠ VERIFIED.
  ...
  // Soft-wires for mid-flight packs may be PRESENT (≠ VERIFIED) or WAITING_DATA — never FAIL.
  for (const name of [
    'er3_soft_wire',
    'er4_soft_wire',
    'er7_soft_wire',
