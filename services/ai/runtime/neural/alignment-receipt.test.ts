...
const context=():AlignmentTrustContext=>({tenantId:'t1',universeId:'u1',target:'OLLAMA',issuerId:'authority',keyId:'k1',publicKeyPem:kp.publicKey.export({type:'spki',format:'pem'}).toString(),expectedSourceRevision:'r1',expectedHumanApprovalRef:'approval-1',expectedTermsDigest:digest,expectedDataPolicyDigest:digest,expectedCapabilityEvidenceDigest:digest,now:new Date('2026-09-13T00:30:00Z'),maxClockSkewMs:30000,replayStore:createInMemoryAlignmentReplayStore(),mode:'TEST'});
...
test('signed approval must match configured approval',()=>assert.equal(evaluateAuthenticatedAlignment({...context(),expectedHumanApprovalRef:'approval-2'},signed()).reason,'human_approval_mismatch'));
test('signed digests must match configured evidence',()=>assert.equal(evaluateAuthenticatedAlignment({...context(),expectedTermsDigest:'b'.repeat(64)},signed()).reason,'evidence_digest_mismatch'));
test('operational mode rejects memory-only replay state',()=>assert.equal(evaluateAuthenticatedAlignment({...context(),mode:'OPERATIONAL'},signed()).reason,'durable_replay_store_required'));
test('operational durable reservation can authorize bounded use',()=>assert.equal(evaluateAuthenticatedAlignment({...context(),mode:'OPERATIONAL',replayStore:{durability:'DURABLE',reserve:()=>true}},signed()).allowed,true));
test('replay store failure is fail closed',()=>assert.equal(evaluateAuthenticatedAlignment({...context(),replayStore:{durability:'DURABLE',reserve:()=>{throw new Error('secret storage detail')}}},signed()).reason,'replay_store_unavailable'));
