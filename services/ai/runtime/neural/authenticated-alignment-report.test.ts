...
const empty=buildAuthenticatedAlignmentReport([]);assert.equal(empty.summary.configuredCount,0);assert.equal(empty.summary.partnershipCount,0);assert.equal(empty.assurance.trustStore,'NOT_CONFIGURED');assert.equal(empty.assurance.auditChain,'NOT_CONNECTED');assert.equal(empty.assurance.independentReview,'REQUIRED');assert.equal(empty.assurance.productionActivation,'DISABLED');assert.equal(JSON.stringify(empty.assurance).includes('key'),false);
