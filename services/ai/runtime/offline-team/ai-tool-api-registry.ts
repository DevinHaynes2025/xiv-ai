...
export function canRouteToTool(tool:ToolCapabilityRecord, classification:DataClass):boolean { if(classification==='TOP_SECRET'&&tool.kind!=='LOCAL_MODEL') return false; if(!tool.tenantScoped||tool.productionAuthority) return false; return tool.state==='CONNECTED'||tool.state==='AVAILABLE'; }
