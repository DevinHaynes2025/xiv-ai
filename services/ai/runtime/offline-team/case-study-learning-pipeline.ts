...
export function ingestCaseStudyLesson(lesson: CaseStudyLesson): CaseStudyLearningCell {
  ...
  const digest = createHash('sha256').update(JSON.stringify({ tenantId: lesson.tenantId, caseId: lesson.caseId, summary: lesson.summary, evidenceRefs: lesson.evidenceRefs })).digest('hex');
  return {
