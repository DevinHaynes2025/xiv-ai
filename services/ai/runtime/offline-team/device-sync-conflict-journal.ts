...
export interface SyncJournalAppend {
  eventId: string; tenantId: string; userId: string; deviceId: string; platform: SyncPlatform; itemId: string;
  action: SyncJournalAction; classification: KnowledgeClassification; baseVersion: number; localVersion: number; remoteVersion: number;
  contentHash?: string; createdAt: string; evidenceRefs: readonly string[];
}
...
export interface ReconciliationInput {
  eventId: string; tenantId: string; userId: string; deviceId: string; platform: SyncPlatform; itemId: string;
  classification: KnowledgeClassification; baseVersion: number; localVersion: number; remoteVersion: number;
  localHash: string; remoteHash: string; createdAt: string; evidenceRefs: readonly string[];
}
...
export interface PlatformSyncMetricsReceipt {
  platform: SyncPlatform; measuredAt: string; attempts: number; successes: number; conflicts: number; pendingPreservedEdits: number;
  successRate: number | null; platformSupportClaim: 'NOT_INFERRED_FROM_SYNC_STATS';
}

export const DEVICE_SYNC_CONFLICT_GUARDRAILS = {
  appendOnlyJournal: true,
  tenantAndUserIsolationRequired: true,
  topSecretOrdinarySyncAllowed: false,
  concurrentEditsAutoOverwriteAllowed: false,
  offlineEditsPreservedByDefault: true,
  platformSupportInferredFromMetrics: false,
...
export class DeviceSyncConflictJournal {
  ...
  evaluateAndRecord(input: ReconciliationInput): ReconciliationDecision {
    ...
    if (input.localHash === input.remoteHash && (localAdvanced || remoteAdvanced)) {
      this.append({ ...input, action: 'RESOLUTION_APPLIED', contentHash: input.localHash });
      return { state: 'RECONCILED_IDENTICAL', requiresHumanReview: false, preserveLocalOfflineEdit: false, nextVersion, reason: 'local and remote payload hashes are identical' };
    }
    if (localAdvanced && remoteAdvanced) {
      this.append({ ...input, action: 'CONFLICT_DETECTED', contentHash: input.localHash });
      this.append({ ...input, eventId: `${input.eventId}:preserve-local`, action: 'OFFLINE_EDIT_PRESERVED', contentHash: input.localHash });
      return { state: 'CONFLICT_PRESERVE_BOTH', requiresHumanReview: true, preserveLocalOfflineEdit: true, nextVersion, reason: 'both local and remote changed after the last common version' };
    }
    if (localAdvanced) {
      this.append({ ...input, action: 'OFFLINE_EDIT_PRESERVED', contentHash: input.localHash });
      return { state: 'PUSH_LOCAL_PENDING', requiresHumanReview: false, preserveLocalOfflineEdit: true, nextVersion: input.localVersion, reason: 'local offline edit remains pending until authorized sync succeeds' };
    }
    if (remoteAdvanced) {
      this.append({ ...input, action: 'REMOTE_EDIT_OBSERVED', contentHash: input.remoteHash });
      return { state: 'APPLY_REMOTE', requiresHumanReview: false, preserveLocalOfflineEdit: false, nextVersion: input.remoteVersion, reason: 'remote changed while local remained at the common base' };
    }
  ...
  buildPlatformMetrics(platform: SyncPlatform, measuredAt: string): PlatformSyncMetricsReceipt {
    ...
    return {
      platform, measuredAt, attempts, successes,
      conflicts: rows.filter(e => e.action === 'CONFLICT_DETECTED').length,
      pendingPreservedEdits: rows.filter(e => e.action === 'OFFLINE_EDIT_PRESERVED').length,
      successRate: attempts === 0 ? null : successes / attempts,
      platformSupportClaim: 'NOT_INFERRED_FROM_SYNC_STATS',
    };
  }
}
