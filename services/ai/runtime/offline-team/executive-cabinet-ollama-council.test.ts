...
assert(EXECUTIVE_ROLES.includes('CTO'));assert(EXECUTIVE_ROLES.includes('CISO'));assert.equal(CABINET_GUARDRAILS.autonomousProductionMutation,false);assert.match(buildExecutivePrompt('CTO','Review architecture'),/Do not expose secrets/);
...
const packet=buildExecutiveDecisionPacket('test objective',mock);assert.equal(packet.requiresHumanApproval,true);assert(packet.dissent.length>=1);
...
const lesson=createApprovedBrainLesson({tenantId:'xiv',sourcePacketHash:'abc',lesson:'Use evidence before scaling.',evidenceRefs:['r'],approved:true});assert.equal(lesson.cellId.length,64);
