...
export function compileExecutiveLessons(lessons: ExecutiveLesson[]): AtomicLearningCell[] {
  if (lessons.length > EXECUTIVE_LEARNING_GUARDRAILS.maxLessonsPerCycle) throw new Error('lesson cycle exceeds bounded maximum');
  return lessons.filter(l => l.approved && l.approvedBy && l.evidenceRefs.length > 0).map(l => {
    ...
    return { cellId: `adc_${createHash('sha256').update(payload).digest('hex')}`, tenantId: l.tenantId, kind: 'EXECUTIVE_LESSON', content: l.content, sourceRole: l.sourceRole, evidenceRefs: [...l.evidenceRefs], approvedBy: l.approvedBy!, createdAt: new Date().toISOString() };
  });
}
