...
export class HistoricalKnowledgeGrowthLedger {
  ...
  snapshot(targetScale: HistoricalGrowthSnapshot['targetScale'] = 'LOCAL_MVP'): HistoricalGrowthSnapshot {
    return this.receipts.reduce((s, r) => ({ receipts: s.receipts + 1, bytes: s.bytes + r.bytes, nodes: s.nodes + r.nodeCount, edges: s.edges + r.edgeCount, targetScale }), { receipts: 0, bytes: 0, nodes: 0, edges: 0, targetScale });
  }
}
