...
export interface IndustryVisualTemplate { industry: string; primaryQuestions: string[]; recommendedVisuals: VisualType[]; narrativeOrder: string[]; }
...
export function getIndustryTemplate(industry: string): IndustryVisualTemplate {
  return INDUSTRY_VISUAL_TEMPLATES.find(x => x.industry === industry) ?? { industry, primaryQuestions:['What happened?','Why?','What should I do?','What changed after action?'], recommendedVisuals:['LINE','BAR','TIMELINE','BEFORE_AFTER'], narrativeOrder:['signal','cause','impact','options','action','outcome'] };
}
