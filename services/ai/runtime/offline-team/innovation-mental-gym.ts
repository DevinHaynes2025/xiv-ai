...
export function evaluateMentalGym(input: { exercise: MentalGymExercise; agentId: string; score: number; evidenceRefs: readonly string[] }): MentalGymResult {
  ...
  return Object.freeze({ exerciseId: input.exercise.exerciseId, agentId: input.agentId, score: boundedScore, evidenceRefs: Object.freeze([...input.evidenceRefs]), passed: boundedScore >= Math.ceil(input.exercise.maxScore * 0.7) && input.evidenceRefs.length > 0 });
}
