...
export const LEADERSHIP_GENOME_GUARDRAILS = Object.freeze({ humanDecision: 'REQUIRED' as const,  biologicalDNA:false, maxGenes:128, evidenceRequired:true, autonomousAuthority:false } as const );
export function compileLeadershipGenome(genes: readonly LeadershipGene[], inheritedByAgents: readonly string[] = []): LeadershipGenome {
  const valid = genes.filter((g) => g.geneId && g.trait.trim() && g.weight >= 0 && g.weight <= 1 && g.evidenceRefs.length > 0).slice(0, LEADERSHIP_GENOME_GUARDRAILS.maxGenes);
