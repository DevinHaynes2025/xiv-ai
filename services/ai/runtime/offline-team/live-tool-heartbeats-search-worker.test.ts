...
const ollama = normalizeHeartbeat({
  ...
  evidenceRefs: ['local:ollama-tags'],
  capabilities: ['local-llm'],
  tenantScoped: true,
  topSecretAllowed: true,
});
...
const lovable = normalizeHeartbeat({
  ...
  health: 'AVAILABLE',
  checkedAt: new Date().toISOString(),
  evidenceRefs: ['plugin:lovable-connected'],
  capabilities: ['ux', 'full-stack'],
  tenantScoped: true,
  topSecretAllowed: true,
});
...
const plan = planAgenticSearch({
  missionId: 'm-44',
  tenantId: 'tenant-a',
  query: 'Find approved lessons about mobile UX and security',
  confidentiality: 'TOP_SECRET',
  preferredSources: ['COMPANY_BRAIN', 'COMMUNITY', 'HISTORY_GRAPH', 'PUBLIC_WEB'],
}, [ollama, lovable]);
