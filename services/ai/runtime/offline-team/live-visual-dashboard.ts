...
export interface VisualEvidencePoint { label: string; value: number; unit?: string; evidenceRefs: string[]; }
export interface StoryVisualCard { id: string; tenantId: string; title: string; story: string; visual: VisualKind; data: VisualEvidencePoint[]; confidence: number; classification: DataClassification; nextAction?: string; }
export interface DashboardLayout { viewport: ViewportKind; columns: number; cards: StoryVisualCard[]; drillDownEnabled: boolean; rawDataDefaultVisible: boolean; }
