...
import { planOllamaToolchain } from './ollama-toolchain';
...
const tools = planOllamaToolchain({ offline: true, requestedRoles: ['CODER','RESEARCHER','TOOLKIT','TERMINAL'] });
assert.equal(tools.some((t) => t.requiresNetwork), false);
assert.equal(tools.length <= 8, true);
console.log('12D-24 mission/genome/family/plugin/toolchain contracts: OK');
