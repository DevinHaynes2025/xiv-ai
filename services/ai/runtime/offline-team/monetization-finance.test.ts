...
const scenario = simulateRevenue({ name: 'base', customers: 100, averageMonthlyRevenuePerCustomerUsd: 299, monthlyChurnRate: 0.05, grossMargin: 0.8, monthlyFixedCostUsd: 10000, monthlyVariableCostPerCustomerUsd: 20 });
assert.equal(scenario.scenarioOnly, true);
assert.equal(scenario.annualizedRevenueUsd > 0, true);

const report = buildMonetizationEvidenceReport({ generatedAt: '2026-09-10T00:00:00.000Z', activeOffers: 4, signedContracts: 0, payingCustomers: 0, monthlyRecurringRevenueUsd: 0, evidenceRefs: [] });
