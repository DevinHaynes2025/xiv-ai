...
export function makeNeuron(input: { tenantId: string; kind: NeuronKind; payload: unknown; evidenceRefs?: readonly string[] }): NeuronNode {
  ...
  return Object.freeze({ neuronId: `n-${hash.slice(0, 24)}`, tenantId: input.tenantId, kind: input.kind, contentHash: hash, evidenceRefs: Object.freeze([...(input.evidenceRefs ?? [])]) });
}
