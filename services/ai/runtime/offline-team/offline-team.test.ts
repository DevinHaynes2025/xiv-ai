...
assert.deepEqual(
  eligibleOfflineSeats(false).map((seat) => seat.id),
  ['LOCAL_RULES', 'REVIEWER', 'LEARNING_RECORDER'],
);
assert.equal(eligibleOfflineSeats(true).length, 4);

const status = buildOfflineTeamStatus({
  ollamaReachable: true,
  grokConfigured: true,
  grokReachable: false,
  runningSeatIds: ['OLLAMA_BUILDER', 'LEARNING_RECORDER'],
  pendingStories: 12,
});
