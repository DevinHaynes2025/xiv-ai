...
export async function runRevenueLab(input: {
  ...
  for (const role of REVENUE_LAB_ROLES) {
    ...
    evaluations.push(Object.freeze({ evaluationId: `${input.runId}:${role}`, runId: input.runId, agentRole: role, skill: 'finance-reasoning', score: response.trim().length > 0 ? 0.6 : 0, evidenceRefs: Object.freeze([evidence]), lesson: 'Preserve assumptions, calculations, risks, and evidence in the next finance round.', createdAt: new Date().toISOString() }));
  }
