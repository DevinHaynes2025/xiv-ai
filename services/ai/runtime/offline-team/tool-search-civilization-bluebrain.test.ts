...
const search=planAgenticSearch({tenantId:'xiv-demo',text:'supplier risk',sources:['COMPANY_BRAIN'],allowOnline:false,classification:'CONFIDENTIAL'},[{source:'COMPANY_BRAIN',ref:'cell:1',summary:'Supplier variability rose',confidence:.88,tenantId:'xiv-demo'}]);
assert.equal(search.answerMode,'LOCAL_ONLY');
const node=createCivilizationNode({label:'Historical innovation example',kind:'IDEA',evidenceRefs:['source:1'],confidence:.8});
assert.equal(node.id.length,64);
const cell=createAtomicPipelineCell({tenantId:'xiv-demo',content:'approved lesson',provenanceRefs:['meeting:1'],classification:'INTERNAL'});
