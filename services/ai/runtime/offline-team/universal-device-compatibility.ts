...
].map(([deviceFamily, osFamily, cpuFamily, supportedSurfaces]) => ({ deviceFamily: deviceFamily as DeviceFamily, osFamily: osFamily as OsFamily, cpuFamily: cpuFamily as CpuFamily, state: 'TARGETED' as CompatibilityState, evidenceRefs: [], offlineMode: true, supportedSurfaces: supportedSurfaces as DeviceCompatibilityProfile['supportedSurfaces'] }));

export function verifyCompatibility(profile: DeviceCompatibilityProfile, evidenceRefs: string[]): DeviceCompatibilityProfile {
